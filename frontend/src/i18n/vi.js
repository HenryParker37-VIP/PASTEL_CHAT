const vi = {
  // Common
  back: '← Quay lại',
  save: 'Lưu thay đổi',
  saving: 'Đang lưu...',
  cancel: 'Hủy',
  close: 'Đóng',
  search: 'Tìm kiếm',
  send: 'Gửi',
  loading: 'Đang tải...',
  online: 'Trực tuyến',
  offline: 'Ngoại tuyến',
  you: 'Bạn',
  logout: 'Đăng xuất',
  signOut: 'Đăng xuất',

  // Header
  appName: 'Pastel Chat',
  onlineCount: (n) => `${n} trực tuyến`,

  // Home
  homeWelcome: (name) => `Chào ${name}!`,
  homeWelcomeAlt: 'Chào mừng trở lại',
  homeTagline: 'Hôm nay bạn muốn làm gì?',
  homeChangeName: 'Đổi tên',
  homeChangeNameDesc: 'Cập nhật tên, mã đăng nhập và khuôn mặt sticker.',
  homeChatFriends: 'Nhắn tin với bạn bè',
  homeChatFriendsDesc: 'Thêm bạn bè bằng mã, đặt biệt danh, bắt đầu trò chuyện riêng tư.',
  homeGroups: 'Nhóm Chat',
  homeGroupsDesc: 'Tạo cuộc trò chuyện nhóm với nhiều bạn bè.',
  homePrivacy: 'Quyền riêng tư & hỗ trợ',
  homePrivacyDesc: 'Thông báo, báo cáo và trợ giúp từ người tạo.',
  homeLoginCode: 'Mã đăng nhập của bạn',
  homeLoginCodeHint: 'Giữ an toàn mã này — đây là cách duy nhất để đăng nhập lại.',
  connected: 'Đã kết nối',
  connecting: 'Đang kết nối...',
  networkOffline: 'Không có kết nối mạng. Tin nhắn sẽ tiếp tục được thử gửi.',
  networkWeak: 'Kết nối yếu. Tin nhắn có thể gửi lâu hơn.',
  notificationsTitle: 'Thông báo',
  notificationsDesc: 'Cập nhật về tin nhắn, bạn bè và hoạt động trong Pastel Chat.',
  notificationsMarkAllRead: 'Đánh dấu đã đọc',
  notificationsLoading: 'Đang tải thông báo...',
  notificationsEmptyTitle: 'Chưa có thông báo',
  notificationsEmptyDesc: 'Các hoạt động mới của bạn bè sẽ xuất hiện tại đây.',
  notificationsUnread: 'Chưa đọc',
  notificationsNewMessage: (name) => `Tin nhắn mới từ ${name || 'bạn bè'}`,
  notificationsFriendRequest: 'Lời mời kết bạn mới',
  notificationsFriendRequestBody: (name) => `${name} đã gửi cho bạn lời mời kết bạn`,
  notificationsFriendAccepted: 'Lời mời kết bạn đã được chấp nhận',
  notificationsFriendAcceptedBody: (name) => `${name} đã chấp nhận lời mời kết bạn`,
  notificationsGroupMessage: 'Tin nhắn nhóm',
  notificationsGroupAdded: 'Bạn đã được thêm vào nhóm',
  notificationsPushTitle: 'Thông báo đẩy',
  notificationsPushDesc: 'Nhận thông báo về tin nhắn mới và hoạt động của bạn bè ngay cả khi Pastel Chat đang đóng.',
  notificationsPushEnable: 'Bật thông báo',
  notificationsPushEnabled: 'Thông báo đẩy đang bật trên thiết bị này',
  notificationsPushDisabled: 'Thông báo đẩy đang tắt',
  notificationsPushBlocked: 'Thông báo bị trình duyệt chặn',
  notificationsPushUnsupported: 'Trình duyệt này không hỗ trợ thông báo đẩy.',
  notificationsPushEnableError: 'Không thể bật thông báo đẩy.',
  notificationsPushDisable: 'Tắt trên thiết bị này',
  notificationsPushIosHint: 'Trên iPhone/iPad, hãy thêm Pastel Chat vào Màn hình chính trước khi bật thông báo đẩy.',
  notificationsPushInstall: 'Hướng dẫn cài đặt PWA',
  releaseTitle: 'Có gì mới',
  releaseDescription: 'Cùng xem những thay đổi mới nhất trong PastelChat.',
  releaseFeatures: 'Tính năng mới',
  releaseFixes: 'Sửa lỗi',
  releaseImprovements: 'Cải thiện',
  releaseViewDetails: 'Xem chi tiết',
  releaseImportant: 'Cập nhật quan trọng',
  releaseEmpty: 'Chưa có ghi chú phát hành.',
  releaseLoadError: 'Không thể tải ghi chú phát hành.',
  releaseNotificationTitle: () => 'PastelChat vừa được cập nhật ✨',
  releaseNotificationBody: (version) => `Phiên bản ${version || ''} đã có sẵn. Xem các tính năng mới và lỗi đã được sửa.`,
  releaseAppUpdateTitle: (version) => `PastelChat v${version} đã sẵn sàng ✨`,
  releaseAppUpdateMessage: 'Có gì mới:',
  profileChatColor: 'Màu bong bóng chat',
  profileChatColorDesc: 'Chọn một màu pastel dịu cho tin nhắn của bạn.',
  profileChatColorAuto: 'Tự động chọn màu pastel',
  profileChatColorSaved: 'Đã lưu màu chat.',
  appUpdateTitle: 'PastelChat đã được cập nhật',
  appUpdateMessage: 'Phiên bản mới đã sẵn sàng. Hãy làm mới trang để cập nhật.',
  appUpdateRefresh: 'Làm mới',
  appUpdateRefreshing: 'Đang làm mới...',
  feedbackConfirmTitle: 'Bạn có chắc không?',
  feedbackFriendRequestSent: 'Đã gửi lời mời kết bạn!',
  feedbackFriendRequestAccepted: 'Đã chấp nhận lời mời kết bạn!',
  feedbackFriendRequestDeclined: 'Đã từ chối lời mời kết bạn.',
  feedbackFriendRemoved: 'Đã xóa bạn bè.',
  feedbackMessageSent: 'Đã gửi tin nhắn',
  feedbackMessageRecalled: 'Đã thu hồi tin nhắn.',
  feedbackMessagePinned: 'Đã lưu tin nhắn.',
  feedbackConversationCleared: 'Đã xóa cuộc trò chuyện.',
  feedbackGroupLeft: 'Bạn đã rời nhóm.',
  feedbackGroupInvited: 'Đã gửi lời mời.',
  feedbackSaved: 'Đã lưu',
  feedbackDeleted: 'Đã xóa.',
  feedbackCopied: 'Đã sao chép',
  feedbackSomethingWrong: 'Có lỗi xảy ra',
  feedbackInvalidImage: 'Vui lòng chọn tệp hình ảnh.',
  feedbackFileTooLarge: 'Tệp quá lớn.',
  feedbackPhotoUploaded: 'Đã tải ảnh lên.',
  feedbackPhotoTooLarge: 'Ảnh quá lớn. Hãy chọn tệp dưới 5 MB.',
  feedbackCallFailed: 'Không thể bắt đầu cuộc gọi. Hãy kiểm tra quyền camera và micro.',
  feedbackSignOutConfirm: 'Bạn sẽ cần mã đăng nhập hoặc phương thức đăng nhập để quay lại.',
  confirm: 'Xác nhận',

  // Login
  loginTitle: 'Pastel Chat',
  loginRegister: 'Đăng ký',
  loginLogin: 'Đăng nhập',
  loginYourName: 'Tên của bạn',
  loginPickFace: 'Chọn khuôn mặt sticker',
  loginNamePlaceholder: 'vd. FluffyBun42',
  loginCreate: 'Tạo tài khoản',
  loginCodeLabel: 'Mã đăng nhập',
  loginCodePlaceholder: 'vd. ABCD-1234',
  loginEnter: 'Vào',
  loginCodeSaved: 'Tôi đã lưu mã rồi!',
  loginSaveCode: 'Lưu mã đăng nhập',
  loginSaveHint: 'Đây là cách duy nhất để quay lại — hãy ghi xuống!',
  loginNewHere: 'Mới ở đây',
  loginHaveCode: 'Có mã rồi',
  loginGoogleTitle: 'Đăng nhập bằng Google',
  loginGoogleDesc: 'Hồ sơ vĩnh viễn · Mã hóa ảnh · Không cần mã',
  loginGoogleRecommended: 'Gợi ý',
  loginOrCreate: 'hoặc tạo tài khoản bằng mã (Thường)',
  loginOrLogin: 'hoặc đăng nhập bằng mã (Thường)',
  loginCodeWarning: 'Bạn sẽ nhận được mã đăng nhập — hãy lưu lại, không có khôi phục qua email.',
  loginPickName: 'Chọn tên',
  loginPickAvatar: 'Chọn hình đại diện',
  loginCreateStandard: 'Tạo tài khoản thường',
  loginCreating: 'Đang tạo...',
  loginChecking: 'Đang kiểm tra...',
  loginLogBackIn: 'Đăng nhập lại',
  loginWelcome: (name) => `Chào mừng, ${name}!`,
  loginSaveWarningTitle: 'Lưu mã này — đây là cách duy nhất để đăng nhập lại!',
  loginSaveWarningBody: 'Không có khôi phục qua email. Nếu mất mã này, tài khoản sẽ mất vĩnh viễn.',
  loginCopyCode: 'Sao chép mã',
  loginShareSave: 'Chia sẻ / Lưu',
  loginConfirmSaved: 'Tôi đã lưu mã đăng nhập an toàn',
  loginEnterApp: 'Vào PastelChat →',
  loginGooglePremium: 'Google Premium',
  loginStandard: 'Thường',
  loginPersistentProfile: 'Hồ sơ vĩnh viễn',
  loginPhotoEncryption: 'Mã hóa ảnh',
  loginNoCodeToSave: 'Không cần lưu mã',
  loginGoogleAvatar: 'Ảnh đại diện Google',
  loginAnonChat: 'Nhắn tin ẩn danh',
  loginCustomAvatar: 'Ảnh đại diện tùy chỉnh',
  loginAllFeatures: 'Đầy đủ tính năng chat',
  loginMustSaveCode: 'Phải lưu mã đăng nhập',
  loginPrevDevice: (name) => `Đã đăng nhập trước đó với ${name}`,
  loginRecoverHint: 'Nhấn để khôi phục mã từ thiết bị này',
  loginRecoverFailed: 'Khôi phục thất bại — tài khoản có thể không còn tồn tại.',
  loginGoogleFailed: 'Đăng nhập Google thất bại. Thử lại hoặc dùng mã đăng nhập.',
  loginNameTaken: 'Tên đã được sử dụng',
  loginPickNameFirst: 'Vui lòng chọn tên',
  loginEnterCode: 'Nhập mã đăng nhập 8 ký tự',

  // Friends
  friendsTitle: 'Bạn bè của bạn',
  friendsPending: 'Lời mời kết bạn',
  friendsAccept: 'Chấp nhận',
  friendsDecline: 'Từ chối',
  friendsSearchPlaceholder: 'Tìm theo tên người dùng hoặc tên...',
  friendsRequest: '＋ Kết bạn',
  friendsAdd: 'Kết bạn',
  friendsRequestSent: 'Đã gửi lời mời',
  friendsAlreadyFriends: 'Bạn bè',
  friendsNoFriends: 'Chưa có bạn bè nào. Tìm kiếm ở trên để thêm ✿',
  friendsRemove: 'Xóa bạn bè này?',
  friendsRemoveTitle: 'Xóa bạn bè?',
  friendsGroups: 'Nhóm',
  friendsNewGroup: '+ Nhóm mới',
  friendsNoGroups: 'Chưa có nhóm nào. Tạo nhóm ở trên ✿',
  friendsCreateGroup: 'Tạo nhóm',
  friendsGroupName: 'Tên nhóm',
  friendsGroupNamePlaceholder: 'vd. Nhóm học tập 🌸',
  friendsAddMembers: 'Thêm thành viên (tùy chọn)',
  friendsMembers: 'thành viên',

  // Chat
  chatSearchPlaceholder: 'Tìm kiếm tin nhắn...',
  chatNoMessages: 'Chưa có tin nhắn',
  chatNoMessagesHint: 'Hãy là người đầu tiên chào!',
  chatMessagePlaceholder: 'Nhắn tin…',
  chatReplyPlaceholder: 'Viết trả lời…',
  chatReplyingTo: 'Trả lời',
  chatVoiceCall: 'Gọi thoại',
  chatVideoCall: 'Gọi video',
  chatHideOnline: '◀ Ẩn',
  chatShowOnline: '▶ Trực tuyến',
  chatOnlineTitle: (n) => `Trực tuyến — ${n}`,
  chatRecallConfirm: 'Thu hồi tin nhắn này?',
  chatClearConfirm: 'Xóa toàn bộ cuộc trò chuyện này? Không thể hoàn tác.',
  chatClearTitle: 'Xóa cuộc trò chuyện?',
  chatRecall: 'Thu hồi',
  chatReply: 'Trả lời',
  chatReact: 'Cảm xúc',
  chatAttach: 'Đính kèm file hoặc ảnh',
  chatFileTooLarge: 'File quá lớn. Tối đa 8 MB.',
  chatViewProfile: 'Xem hồ sơ',
  chatOnlineNow: 'Đang trực tuyến',

  // Group chat
  groupMembers: 'Thành viên',
  groupLeave: 'Rời nhóm',
  groupLeaveConfirm: 'Rời khỏi nhóm này?',
  groupAdd: 'Thêm thành viên',
  groupCreatedBy: 'Tạo bởi',
  groupInvite: 'Mời',
  groupTypeMessage: 'Nhắn tin nhóm…',
  groupInfo: 'Thông tin nhóm',

  // Profile
  profileTitle: 'Hồ sơ của bạn',
  profileName: 'Tên hiển thị',
  profileBio: 'Giới thiệu',
  profileBioPlaceholder: 'Kể cho bạn bè nghe một chút về bạn...',
  profileStatus: 'Trạng thái',
  profileStatusPlaceholder: 'Hoặc nhập trạng thái tùy chỉnh...',
  profileFace: 'Khuôn mặt sticker',
  profileSaved: 'Đã lưu! ✿',
  profileLoginCode: 'Mã:',

  // Status presets
  statusAvailable: 'Sẵn sàng',
  statusBusy: 'Bận',
  statusAway: 'Vắng mặt',
  statusGaming: 'Đang chơi game',
  statusListening: 'Đang nghe nhạc',
  statusStudying: 'Đang học',
  statusWorking: 'Đang làm việc',
  statusEating: 'Đang ăn',
  statusTraveling: 'Đang du lịch',
  statusWorkingOut: 'Đang tập thể dục',

  // Install guide
  homeInstall: 'Cài đặt trên màn hình chính',
  addToHomeScreen: 'Thêm vào màn hình chính',
  homeInstallDesc: 'Lưu ứng dụng vào điện thoại để truy cập nhanh.',
  homeSharedPhotos: 'Chia sẻ Ảnh',
  homeSharedPhotosDesc: 'Chia sẻ và xem ảnh với bạn bè thân thiết theo thời gian thực.',
  installTitle: 'Cài đặt Pastel Chat',
  installSubtitle: 'Thêm ứng dụng vào màn hình chính để truy cập nhanh.',
  installIos: 'iPhone & iPad',
  installAndroid: 'Android',
  installStep: (n) => `Bước ${n}`,
  installIosSteps: (url) => [
    { step: '1', title: 'Mở bằng Safari', desc: `Dùng trình duyệt Safari để mở Pastel Chat tại ${url}.` },
    { step: '2', title: 'Nhấn nút Chia sẻ', desc: 'Nhấn biểu tượng Chia sẻ (hình vuông có mũi tên hướng lên).' },
    { step: '3', title: 'Thêm vào Màn hình chính', desc: 'Kéo xuống và chọn “Thêm vào Màn hình chính”.' },
    { step: '4', title: 'Xác nhận', desc: 'Đặt tên ứng dụng là “Pastel Chat”, sau đó nhấn “Thêm”.' }
  ],
  installAndroidSteps: (url) => [
    { step: '1', title: 'Mở bằng Chrome', desc: `Dùng trình duyệt Chrome để mở Pastel Chat tại ${url}.` },
    { step: '2', title: 'Mở menu', desc: 'Nhấn nút menu (biểu tượng ba dấu chấm dọc).' },
    { step: '3', title: 'Cài đặt ứng dụng', desc: 'Chọn “Cài đặt ứng dụng” hoặc “Thêm vào màn hình chính”.' },
    { step: '4', title: 'Hoàn tất', desc: 'Pastel Chat sẽ được thêm vào màn hình chính.' }
  ],
  installDone: 'Xong! Mở Pastel Chat từ màn hình chính.',

  // Private space
  homeMySpace: 'Không gian riêng của tôi',
  homeMySpaceDesc: 'Ghi chú, ảnh, nhắc nhở và sinh nhật bạn bè.',
  mySpaceTitle: 'Không gian riêng của tôi',
  mySpaceNotes: 'Ghi chú',
  mySpacePhotos: 'Ảnh',
  mySpaceBirthdays: 'Sinh nhật',
  mySpaceReminders: 'Nhắc nhở',
  mySpaceNewNote: '+ Ghi chú mới',
  mySpaceNewPhoto: '+ Thêm ảnh',
  mySpaceNewReminder: '+ Nhắc nhở mới',
  mySpaceNewBirthday: '+ Thêm sinh nhật',
  mySpaceNoNotes: 'Chưa có ghi chú nào. Tạo một để bắt đầu ✿',
  mySpaceNoPhotos: 'Chưa có ảnh nào. Tải lên để chia sẻ với bạn bè ✿',
  mySpaceNoBirthdays: 'Chưa lưu sinh nhật nào ✿',
  mySpaceNoReminders: 'Chưa có nhắc nhở nào ✿',
  mySpaceNoteTitle: 'Tiêu đề ghi chú',
  mySpaceNoteContent: 'Bạn đang nghĩ gì?',
  mySpaceShareWith: 'Chia sẻ với (tùy chọn)',
  mySpaceSelectFriend: 'Chọn bạn bè...',
  mySpaceSharePublic: 'Công khai (tất cả bạn bè)',
  mySpaceSharePrivate: 'Riêng tư (chỉ bạn)',
  mySpaceDelete: 'Xóa',
  mySpaceDeleteConfirm: 'Xóa cái này? Không thể hoàn tác.',
  mySpaceReminderDate: 'Ngày',
  mySpaceReminderTime: 'Giờ',
  mySpaceReminderText: 'Văn bản nhắc nhở',
  mySpaceUpcoming: 'Sắp tới',
  mySpaceFriendName: 'Tên bạn bè',
  mySpaceDescription: 'Ghi chú nhanh cho ghi chú hoặc nội dung được chia sẻ',

  // Tutorial
  tutorialReplay: 'Xem lại hướng dẫn chào mừng',
  tutorialReplayDesc: 'Xem lại hướng dẫn bắt đầu từ đầu.',
  tutorialReplayBtn: 'Xem lại Hướng dẫn',

  // Privacy & support
  privacyAppearance: 'Giao diện',
  privacyAppearanceDesc: 'Chọn giữa chế độ sáng và tối để có trải nghiệm thoải mái hơn.',
  privacyLight: 'Chế độ sáng',
  privacyDark: 'Chế độ tối',
  privacyToggleTheme: 'Chuyển chế độ sáng/tối',
  privacyLanguage: 'Ngôn ngữ',
  privacyLanguageDesc: 'Chọn ngôn ngữ hiển thị ưa thích của bạn.',
  privacyNotifications: 'Thông báo đẩy',
  privacyNotificationsDesc: 'Nhận thông báo tức thì cho tin nhắn mới và lời mời kết bạn, ngay cả khi PastelChat đang đóng hoặc chạy nền.',
  privacyNotificationsEnableBtn: 'Bật thông báo đẩy',
  privacyNotificationsEnabled: 'Thông báo đẩy đang hoạt động trên thiết bị này',
  privacyNotificationsTestBtn: 'Gửi thông báo thử nghiệm',
  privacyNotificationsDisableBtn: 'Tắt trên thiết bị này',
  privacyNotificationsDenied: 'Thông báo bị chặn trong cài đặt trình duyệt',
  privacyNotificationsDeniedHint: 'Vui lòng cho phép thông báo cho PastelChat trong Cài đặt của trình duyệt hoặc thiết bị.',
  privacyNotificationsUnsupported: 'Trình duyệt này không hỗ trợ thông báo đẩy.',
  privacyNotificationsIosHint: 'Trên iPhone/iPad, hãy thêm PastelChat vào Màn hình chính trước để bật thông báo đẩy.',
  privacyNotificationsIosGuide: 'Hướng dẫn cài đặt PWA',
  privacyNotificationsTestSuccess: 'Đã gửi thông báo thử nghiệm! Hãy kiểm tra thanh thông báo.',
  privacyWhatWeStore: 'Chúng tôi lưu trữ gì',
  privacyStore1: 'Tên, khuôn mặt sticker và mã đăng nhập của bạn (cần thiết để tìm lại bạn)',
  privacyStore2: 'Danh sách bạn bè với biệt danh bạn đã đặt',
  privacyStore3: 'Tin nhắn của bạn (để bạn có thể cuộn lại) — có thể xóa bất cứ lúc nào từ bảng bên phải',
  privacyNoAds: 'Không có gì được chia sẻ ra ngoài PastelChat. Không quảng cáo, không theo dõi ✿',
  privacyFeedbackTitle: 'Tìm thấy lỗi hoặc có ý tưởng?',
  privacyFeedbackDesc: 'Tin nhắn của bạn sẽ được gửi thẳng đến người tạo — Nguyễn Mạnh Tuấn Hùng (Henry Parker).',
  privacyFeedbackPlaceholder: 'Hãy cho người tạo biết bạn đang nghĩ gì...',
  privacyFeedbackEmpty: 'Vui lòng mô tả những gì đã xảy ra',
  privacyFeedbackError: 'Không thể gửi. Thử lại sớm nhé.',
  privacyFeedbackSent: 'Đã gửi! Cảm ơn bạn đã giúp PastelChat tốt hơn.',
  privacyFeedbackAnother: 'Gửi thêm',
  privacyFeedbackSending: 'Đang gửi...',
  privacyFeedbackSend: 'Gửi cho người tạo',

  // Character Studio
  customizeLyra: 'Tùy chỉnh Lyra',
  characterStudioTitle: 'Xưởng Nhân Vật',
  characterStudioSubtitle: 'Định hình cách Lyra trò chuyện, suy nghĩ và gắn bó với bạn',
  tabIdentity: 'Nhận dạng',
  tabPersonality: 'Tính cách',
  tabSpeech: 'Cách nói chuyện',
  tabMind: 'Cách suy nghĩ',
  tabRules: 'Nên & Không nên',
  tabRelationship: 'Mối quan hệ',
  tabLore: 'Kiến thức & Bối cảnh',
  tabExamples: 'Hội thoại mẫu',
  tabPreview: 'Thử nghiệm',
  resetCustomization: 'Khôi phục mặc định',
  resetCustomizationConfirm: 'Đặt lại tùy chỉnh Lyra về mặc định? Ký ức, lịch sử trò chuyện và ảnh đại diện của bạn sẽ không bị ảnh hưởng.',
  saveCustomization: 'Lưu thay đổi',
  customizationSaved: 'Đã lưu tùy chỉnh Lyra của bạn!',
  customizationReset: 'Đã khôi phục Lyra về mặc định.',
  studioRulesTitle: 'Quy tắc hành vi & Giới hạn',
  studioRulesSubtitle: 'Xác định rõ những điều Lyra của bạn nên và không nên làm. Các quy tắc này định hướng phản hồi mà không vi phạm sự an toàn hay tính chính xác.',
  studioLyraShould: 'Lyra nên làm',
  studioLyraShouldDesc: 'Thói quen, phản ứng tự nhiên và phong cách bạn muốn Lyra thực hiện.',
  studioLyraShouldPlaceholder: 'Ví dụ:\n• Nói chuyện tự nhiên, ngắn gọn\n• Thỉnh thoảng trêu đùa nhẹ nhàng khi phù hợp\n• Nhận sai thẳng thắn khi được sửa\n• Phản hồi tự nhiên thay vì trả lời như trợ lý ảo',
  studioLyraShouldNot: 'Lyra không nên làm',
  studioLyraShouldNotDesc: 'Thói quen, hành vi và giới hạn bạn muốn Lyra tuyệt đối tránh.',
  studioLyraShouldNotPlaceholder: 'Ví dụ:\n• Không trả lời như một trợ lý ảo chung chung\n• Không dùng quá nhiều emoji\n• Không đặt câu hỏi liên tục sau mỗi tin nhắn\n• Không tự bịa đặt kỷ niệm cá nhân\n• Không liên tục nhắc mình là AI',
  studioLocationTitle: 'Địa điểm & Múi giờ (Tùy chọn)',
  studioLocationDesc: 'Thiết lập thành phố hoặc múi giờ riêng cho Lyra. Khi bạn hỏi "chỗ cậu mấy giờ rồi?", Lyra sẽ dùng múi giờ này. Nếu để trống, Lyra sẽ chia sẻ múi giờ của bạn.',
  studioLocation: 'Vị trí của Lyra',
  studioLocationPlaceholder: 'Ví dụ: London, Vương quốc Anh hoặc Tokyo, Nhật Bản',
  studioTimezone: 'Múi giờ của Lyra',
  studioTimezonePlaceholder: 'Ví dụ: Europe/London, Asia/Tokyo, America/New_York'
};

export default vi;
