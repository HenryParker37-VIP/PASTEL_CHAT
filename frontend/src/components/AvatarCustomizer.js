...
function initStickerState(avatarUrl) {
  try {
    ...
    const bgColor = bgM ? bgM[1] : 'ffb6c1';
    return { seed, bgColor };
  } catch {
...
const AvatarCustomizer = ({ currentAvatarUrl, onAvatarChange, compact = false }) => {
  ...
  const currentCharUrl = getCharacterAvatarUrl(charOpts);
  const previewUrl = mode === 'sticker' ? currentStickerUrl : currentCharUrl;

  const commit = useCallback((url) => { onAvatarChange(url); }, [onAvatarChange]);
          ...
          <div style={{
            ...
            {AVATARS.map(av => (
              <button
                ...
                title={av.seed}
                style={{
                  padding: 4, borderRadius: 12, border: '2px solid',
      ...
      {mode === 'character' && (
        <div style={{ maxHeight: compact ? 300 : 420, overflowY: 'auto', paddingRight: 2 }}>
          ...
          <SwatchRow label="Hair Color" items={HAIR_COLORS} selected={charOpts.hairColor}
            onSelect={item => handleCharChange({ hairColor: item.hex })} size={24} />
          <OptionRow label="Hair Style" items={HAIR_STYLES} selected={charOpts.hairStyle}
          ...
          <OptionRow label="Outfit" items={OUTFITS} selected={charOpts.outfit}
            onSelect={item => handleCharChange({ outfit: item.value })} />
          {/* Earrings */}
