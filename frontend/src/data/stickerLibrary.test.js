...
describe('active PastelChat sticker library', () => {
  test('contains first-party characters and the full category vocabulary', () => {
    expect(LOCAL_STICKER_PACKS).toHaveLength(1);
    expect(LOCAL_STICKERS.length).toBeGreaterThanOrEqual(50);
  ...
  test('uses accentless English/Vietnamese search aliases', () => {
    expect(stickerMatchesSearch(LOCAL_STICKERS.find(sticker => sticker.id === 'bunny_camon'), 'cam on')).toBe(true);
    expect(stickerMatchesSearch(LOCAL_STICKERS.find(sticker => sticker.id === 'bunny_hello'), 'xin chao')).toBe(true);
    expect(stickerMatchesSearch(LOCAL_STICKERS.find(sticker => sticker.id === 'bunny_shocked'), 'OMG')).toBe(true);
  });
});
